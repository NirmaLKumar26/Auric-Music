package nirmal.auric.music.auricmusic.updater.downloadmanager
...
class UpdateDownloadWorker(private val context: Context, workerParams: WorkerParameters) :
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            val downloadFile = if (isZip) File(downloadDir, "auric_temp.zip") else File(downloadDir, "auricmusic.apk")
            ...
            val finalFile = if (isZip) {
                val targetApkFile = File(downloadDir, "auricmusic.apk")
                var extracted = false
                try {
