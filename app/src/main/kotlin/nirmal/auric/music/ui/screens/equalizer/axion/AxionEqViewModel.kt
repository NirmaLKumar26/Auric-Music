...
import nirmal.auric.music.eq.data.FilterType
import nirmal.auric.music.eq.data.ParametricEQBand
...
class AxionEqViewModel @Inject constructor(
    ...
    private val prefs = context.getSharedPreferences("auric_eq_prefs", Context.MODE_PRIVATE)
    ...
    val customProfiles = eqProfileRepository.profiles.map { profiles ->
        profiles.filter { it.isCustom && it.id != "auric_tuning" }
