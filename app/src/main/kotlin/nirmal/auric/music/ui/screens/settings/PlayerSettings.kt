...
import nirmal.auric.music.constants.EnableGoogleCastKey
...
import nirmal.auric.music.constants.EnableExportAsMp3Key
...
highlightKey: String? = null) {
    ...
    val (dataSaverEnabled, onDataSaverEnabledChange) = rememberPreference(
        key = nirmal.auric.music.constants.DataSaverEnabledKey,
        defaultValue = false
    )
    ...
    val (downloadQuality, onDownloadQualityChange) = rememberEnumPreference(
        nirmal.auric.music.constants.DownloadQualityKey,
        defaultValue = nirmal.auric.music.constants.DownloadQuality.YOUTUBE
    )
    ...
    if (showDownloadQualityDialog) {
        EnumDialog(
            ...
            values = listOf(nirmal.auric.music.constants.DownloadQuality.YOUTUBE),
            valueText = {
                when (it) {
                    nirmal.auric.music.constants.DownloadQuality.YOUTUBE -> "YouTube Music (AAC/Default)"
                    else -> ""
                }
            }
        )
    ...
    if (showPlaybackEngineDialog) {
        EnumDialog(
            ...
            onSelect = {
                ...
                nirmal.auric.music.utils.YTPlayerUtils.playbackEngine = it
                showPlaybackEngineDialog = false
            },
            title = "Playback Engine",
            current = playbackEngine,
            values = listOf(
                nirmal.auric.music.constants.PlaybackEngine.POTOKEN,
                nirmal.auric.music.constants.PlaybackEngine.BRAVEPIPE,
                nirmal.auric.music.constants.PlaybackEngine.AUTO,
            ),
            valueText = {
                when (it) {
                    nirmal.auric.music.constants.PlaybackEngine.POTOKEN -> "PoToken (Recommended)"
                    nirmal.auric.music.constants.PlaybackEngine.BRAVEPIPE -> "BravePipe (NewPipe)"
                    nirmal.auric.music.constants.PlaybackEngine.AUTO -> "Auto (Try Both)"
                }
            },
            valueDescription = {
                when (it) {
    ...
    isHighlighted = (highlightKey == stringResource(R.string.seek_seconds_addup)),
                ...
                add(Material3SettingsItem(
    isHighlighted = (highlightKey == stringResource(R.string.auric_equalizer)),
                    icon = painterResource(R.drawable.auricequlizer),
                    title = { Text(stringResource(R.string.auric_equalizer)) },
                    description = { Text(stringResource(R.string.auric_equalizer_desc)) },
