...
### UI rule: Custom Auric Music Aesthetic (over Material You)

All UI work — new screens, new components, and edits to existing ones —
**must** match the established custom visual style of Auric Music rather than blindly
...
hub for all design decisions, component usage, and style choices in the Auric Music codebase.
...
- Exact current contents of `nirmal/auric/music/auricmusic/` (contains core app-level initializers/application class) and `:unison` (cross-cutting/shared utilities and common base classes) — check source before editing.

## What this app is

Auric Music is a native **Android** music streaming client (Kotlin + Jetpack
Compose) that streams from YouTube Music's catalog ad-free, and layers on
offline downloads, real-time synced lyrics, music recognition ("Auric Find"),
...
Package/namespace: `nirmal.auric.music` (app module). Application ID matches.
...
| `:shazamkit` | Music recognition ("Auric Find") |
| `:canvas`, `:auricmusiccanvas`, `:applecanvas` | Canvas-style looping video backgrounds for tracks (different providers) |
...
Path: `app/src/main/kotlin/nirmal/auric/music/` (note: source dir is `kotlin/`,
...
auricmusic/      Core app-level classes (Application class, core initializers)
eq/             Equalizer
...
recognition/    Music recognition (Auric Find) app-side logic
